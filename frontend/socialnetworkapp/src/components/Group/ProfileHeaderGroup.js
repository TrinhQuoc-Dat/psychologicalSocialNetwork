...
import FollowButtonGroup from "./FollowButtonGroup";
import { followingGroupCount, updateGroupAvatarOrCover } from "../../services/groupService";

const ProfileHeaderGroup = ({ group }) => {
  ...
  const handleCrop = async () => {
    ...
    try {
      await updateGroupAvatarOrCover(formData, group.id);
  ...
  useEffect(() => {
    const UserFollower = async () => {
      try {
        const res = await followingGroupCount(group.id);
        ...
        <div className="absolute top-[10rem] -bottom-16 left-4 flex items-end gap-6 w-[calc(100%-2rem)] pr-4">
          ...
          <div className="flex justify-between items-center flex-1 flex-wrap">
            <div>
              ...
              <p className="text-gray-600 text-sm">{follower} Người theo dõi</p>
            </div>
            <div className="flex gap-2 mt-3 sm:mt-0">
              <FollowButtonGroup groupId={group.id} />
...
export default ProfileHeaderGroup;
